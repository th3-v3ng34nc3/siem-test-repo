...
def get_env_config():
    return {
        ...
        "chunk_size": int(os.environ.get("CHUNK_SIZE_LIMIT") or 3 * 1024 * 1024),
        "line_limit": int(os.environ.get("LINE_LIMIT") or 1000),
...
def handler(event, context):
    ...
    for record in event.get('Records', []):
        ...
        # AWS Config periodically writes a 0-byte file here to verify it can
        # write to the bucket. It matches the same key prefix as real
        # snapshot/history files but isn't gzip/JSON, so skip it explicitly
        # instead of logging a confusing parse error for a non-payload object.
        if key.endswith("ConfigWritabilityCheckFile"):
            logger.info(f"Skipping AWS Config's writability-check file: {key}")
            continue
        ...
        try:
            ...
            # 3. Process the 'configurationItems' list.
            #    AWS Config snapshot AND history files both use this key
            #    (this is the only line that differs from the CloudTrail
            #    variant of this Lambda, which reads data['Records']).
            items = data.get('configurationItems', [])
            logger.info(f"Loaded {len(items)} configurationItems from {key}")
