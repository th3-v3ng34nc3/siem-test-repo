...
def get_env_config():
    return {
        ...
        "log_group_labels": json.loads(os.environ.get("LOG_GROUP_LABELS", "{}"))
    }
...
def lambda_handler(event, context):
    ...
    log_source = config['log_group_labels'].get(log_group, log_group)
    ...
    for i, entry in enumerate(log_events):
        entry = {**entry, "log_group": log_group, "log_source": log_source}
