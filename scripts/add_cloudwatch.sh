...
CLOUDWATCH_TPL="$TPL_LOC/aws-cloudwatch-collector-tpl"
...
read -p "Do you want to add CloudWatch integration $INTEGRATION_ID to tenant $TENANT_ID in $ENV_ID? (y/n): " CONFIRM
...
cp -r $CLOUDWATCH_TPL $INTEGRATION_TPL
if [ $? -eq 0 ]; then
